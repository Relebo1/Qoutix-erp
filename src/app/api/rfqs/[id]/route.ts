...
// Update status or award supplier
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Record a supplier's quoted price
  if (body.type === "RECORD_QUOTE") {
    ...
    // Auto-advance status to RESPONSES_RECEIVED if still SENT
  ...
  if (body.status) {
    await prisma.rFQ.update({
      where: { id: Number(id) },
      data: { status: body.status as RFQStatus },
    });
    return NextResponse.json({ ok: true });
  }
