import { NextRequest, NextResponse } from "next/server";
import { cookies } from "next/headers";
import { prisma } from "@/lib/prisma";
import { verifyToken } from "@/lib/auth";
import { InvoiceStatus } from "@prisma/client";
import { autoPayOnStatusChange } from "@/lib/payments";

async function getPayload() {
  const cookieStore = await cookies();
  const token = cookieStore.get("token")?.value;
  if (!token) return null;
  try { return await verifyToken(token); } catch { return null; }
}

export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const payload = await getPayload();
  if (!payload) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const { id } = await params;
  const { status } = await req.json();

  const invoice = await prisma.invoice.findFirst({
    where: { id: Number(id), companyId: payload.companyId },
  });
  if (!invoice) return NextResponse.json({ error: "Not found" }, { status: 404 });

  // If manually marking as PAID, auto-generate a payment for the remaining balance
  if (status === InvoiceStatus.PAID && invoice.status !== InvoiceStatus.PAID) {
    await autoPayOnStatusChange(Number(id), payload.companyId, Number(payload.sub));
    // autoPayOnStatusChange already sets status to PAID via recordPayment
    const updated = await prisma.invoice.findUnique({ where: { id: Number(id) } });
    return NextResponse.json({ invoice: updated });
  }

  const updated = await prisma.invoice.update({
    where: { id: Number(id) },
    data: { status: status as InvoiceStatus },
  });

  return NextResponse.json({ invoice: updated });
}

export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const payload = await getPayload();
  if (!payload) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const { id } = await params;
  const { clientId, currency, issueDate, dueDate, plannedSendDate, discount, notes, items, status, accentColor, bgColor, fontColor, fontFamily } = await req.json();

  const invoice = await prisma.invoice.findFirst({
    where: { id: Number(id), companyId: payload.companyId },
  });
  if (!invoice) return NextResponse.json({ error: "Not found" }, { status: 404 });

  const subtotal = items.reduce((s: number, i: { quantity: number; unitPrice: number }) => s + i.quantity * i.unitPrice, 0);
  const discountAmt = (subtotal * (discount ?? 0)) / 100;
  const tax = items.reduce((s: number, i: { quantity: number; unitPrice: number; taxRate: number }) => s + i.quantity * i.unitPrice * (1 - (discount ?? 0) / 100) * (i.taxRate / 100), 0);
  const total = subtotal - discountAmt + tax;

  await prisma.invoiceItem.deleteMany({ where: { invoiceId: Number(id) } });

  const updated = await prisma.invoice.update({
    where: { id: Number(id) },
    data: {
      clientId,
      currency,
      issueDate: new Date(issueDate),
      dueDate: new Date(dueDate),
      plannedSendDate: plannedSendDate ? new Date(plannedSendDate) : null,
      subtotal,
      discount: discountAmt,
      tax,
      total,
      notes: notes || null,
      status: status as InvoiceStatus,
      accentColor: accentColor || null,
      bgColor: bgColor || null,
      fontColor: fontColor || null,
      fontFamily: fontFamily || null,
      items: {
        create: items.map((i: { description: string; quantity: number; unitPrice: number; taxRate: number }) => ({
          description: i.description,
          quantity: i.quantity,
          price: i.unitPrice,
          tax: i.quantity * i.unitPrice * (i.taxRate / 100),
          amount: i.quantity * i.unitPrice,
        })),
      },
    },
  });

  return NextResponse.json({ invoice: updated });
}
