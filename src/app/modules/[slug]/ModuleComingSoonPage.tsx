...
import { ArrowLeft, Check, Bell, ArrowRight } from "lucide-react";
import { ModuleIcon } from "@/components/ModuleSelector";
import type { AppModule } from "@/lib/modules";

export default function ModuleComingSoonPage({ module: mod }: { module: AppModule }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-16" style={{ backgroundColor: "var(--bg)" }}>
      <div className="w-full max-w-lg">
        {/* Back */}
        <Link
          href="/"
          className="inline-flex items-center gap-1.5 text-sm mb-8 hover:text-blue-600 transition-colors"
        ...
        <div className="rounded-2xl border p-8 shadow-sm" style={{ backgroundColor: "var(--bg-card)", borderColor: "var(--border)" }}>
          ...
          <div className="text-center mb-8">
            <div className="w-16 h-16 rounded-2xl bg-blue-50 dark:bg-blue-950 border border-blue-100 dark:border-blue-900 flex items-center justify-center mx-auto mb-4 text-blue-600">
            ...
            <ul className="space-y-2.5">
              {mod.features.map((f) => (
                <li key={f} className="flex items-center gap-2.5 text-sm" style={{ color: "var(--text-secondary)" }}>
                  <span className="w-5 h-5 rounded-full bg-emerald-50 dark:bg-emerald-950 border border-emerald-200 dark:border-emerald-800 flex items-center justify-center flex-shrink-0">
        ...
        <p className="text-center text-xs mt-6" style={{ color: "var(--text-muted)" }}>
          Quotix is growing into a complete business platform.{" "}
