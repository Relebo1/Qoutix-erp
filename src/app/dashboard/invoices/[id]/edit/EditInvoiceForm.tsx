...
interface Props {
  ...
  clients: Client[];
  company: { name: string; logo: string | null; email: string; phone: string; address: string; currency: string; };
}

const emptyItem = (): LineItem => ({ description: "", quantity: 1, unitPrice: 0, taxRate: 0 });

export default function EditInvoiceForm({ invoice, clients, company }: Props) {
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  const [accentColor, setAccentColor] = useState(invoice.accentColor);
  const [bgColor, setBgColor] = useState(invoice.bgColor);
  const [fontColor, setFontColor] = useState(invoice.fontColor);
  const [fontFamily, setFontFamily] = useState(invoice.fontFamily);
  ...
  const [plannedSendDate, setPlannedSendDate] = useState(invoice.plannedSendDate ?? "");
  const [discount, setDiscount] = useState(invoice.discount);
  const [notes, setNotes] = useState(invoice.notes);
  const [items, setItems] = useState<LineItem[]>(invoice.items.length ? invoice.items : [emptyItem()]);
  ...
  const handleSave = async (status: "DRAFT" | "SENT") => {
    ...
    try {
      const res = await fetch(`/api/invoices/${invoice.id}`, {
        ...
        body: JSON.stringify({ clientId: Number(clientId), currency, issueDate, dueDate, plannedSendDate: plannedSendDate || null, discount, notes, items, status, accentColor, bgColor, fontColor, fontFamily }),
      });
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-5 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold" style={{ color: "var(--text-primary)" }}>Edit Invoice</h1>
        </div>
