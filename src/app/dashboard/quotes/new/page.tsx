...
import { QuotationDocType } from "@prisma/client";
import DateInput from "@/components/DateInput";

const CURRENCIES = ["LSL","ZAR","USD","GBP","EUR"];
const DOC_TYPES = [
  { value: QuotationDocType.QUOTATION,        label: "Quotation" },
  { value: QuotationDocType.PROFORMA_INVOICE, label: "Proforma Invoice" },
  { value: QuotationDocType.SALES_ORDER,      label: "Sales Order" },
];

interface Client { id: number; companyName: string; contactName: string; email: string | null; address: string | null; }
interface Product { id: number; name: string; price: number; taxRate: number; }
...
export default function NewQuotePage() {
  ...
  const [docType, setDocType] = useState<QuotationDocType>(QuotationDocType.QUOTATION);
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-5 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold" style={{ color: "var(--text-primary)" }}>New Quotation</h1>
        </div>
      ...
      <div className="flex gap-6 flex-1 min-h-0 overflow-hidden">
        ...
        <div className="w-[420px] flex-shrink-0 overflow-y-auto space-y-5 pr-1">
          ...
          <section className="rounded-xl border p-5 space-y-3" style={{ backgroundColor: "var(--bg-card)", borderColor: "var(--border)" }}>
            <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Document Type</p>
            <div className="flex gap-2">
              {DOC_TYPES.map((t) => (
                <button key={t.value} onClick={() => setDocType(t.value)}
                  className={`flex-1 py-2 px-2 rounded-lg border text-xs font-semibold transition-colors ${docType === t.value ? "bg-blue-600 text-white border-blue-600" : "hover:border-blue-400"}`}
                  style={docType === t.value ? {} : { borderColor: "var(--border)", color: "var(--text-secondary)", backgroundColor: "var(--bg)" }}>
                  {t.label}
                </button>
              ))}
            </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <DocumentTemplate
            type="QUOTATION"
            docLabel={DOC_TYPES.find(t => t.value === docType)?.label.toUpperCase()}
            accentColor={accentColor}
            bgColor={bgColor}
            fontColor={fontColor}
            fontFamily={fontFamily}
