...
import { ChevronLeft, Loader2, Building2, User, Mail, Phone, MapPin, Briefcase, FileText } from "lucide-react";
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-lg font-bold" style={{ color: "var(--text-primary)" }}>Add Client</h1>
      </div>

      {error && (
        <div className="mb-5 px-4 py-3 rounded-lg bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-900 text-red-700 dark:text-red-400 text-sm">{error}</div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex items-center justify-end gap-3 pt-1">
          <Link href="/dashboard/clients" className="text-sm font-semibold px-5 py-2.5 rounded-lg border transition-colors" style={{ borderColor: "var(--border)", color: "var(--text-secondary)", backgroundColor: "var(--bg-card)" }}>
            Cancel
          </Link>
